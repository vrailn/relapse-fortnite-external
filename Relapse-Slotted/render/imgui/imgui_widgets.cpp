...
bool ImGui::Checkbox(const char* label, bool* v, ImVec2 loc_offset) {
    ImGuiWindow* window = GetCurrentWindow();
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImRect frame_bb(pos - loc_offset + ImVec2(w - size.x, 0), pos - loc_offset + ImVec2(w, size.y));
    ...
    alpha = ImLerp(alpha, *v ? 1.0f : 0.5f, 0.01f);

    auto frame_inactive = ImGui::GetColorU32(rgb_to_imvec4(12, 12, 12, 255));
    auto frame_active = ImGui::GetColorU32(rgb_to_imvec4(10, 10, 10, value->second));
    auto accent_color = ImColor(0, 0, 255, 255);
    auto text_disabled = ImColor(0, 0, 230, 255);

    draw->AddText(ImVec2(total_bb.Min.x, frame_bb.GetCenter().y - label_size.y / 2), ImGui::GetColorU32(ImVec4(1.f, 1.f, 1.f, alpha)), label);

    draw->AddRectFilled(frame_bb.Min, frame_bb.Max, frame_inactive, 10);
    draw->AddRectFilled(frame_bb.Min, frame_bb.Max, frame_active, 10);

    //draw->AddRect(frame_bb.Min, frame_bb.Max, gui.border.to_im_color(), 10);

    draw->AddCircleFilled(ImVec2(frame_bb.Min.x + 8 + (14 * value->second), frame_bb.GetCenter().y),
        7, *v ? accent_color : text_disabled, 30);
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags ) {
    ...
    ImVec2 label_size = CalcTextSize( label, 0, 1 );
    ImVec2 size = { 100, label_size.y + style.FramePadding.y * 2 - 5 };

    float w = GetWindowWidth( ) - 20;
    ...
    auto frame_active = ImGui::GetColorU32(rgb_to_imvec4(10, 10, 10, value->second));
    auto accent_color = ImColor(0, 0, 255, 255);
    auto text_disabled = ImColor(255, 255, 255, 255);
    auto text_disabled_imvec4 = ImVec4(1, 1, 1, 1);
    auto text = ImColor(255, 255, 255, 255);

    draw->AddRectFilled( bb.Min, bb.Max, frame_inactive, 3 );
    draw->AddRectFilled( bb.Min, bb.Max, frame_active, 3 );

    if ( preview_value != NULL && !( flags & ImGuiComboFlags_NoPreview ) ) {

        PushStyleColor( ImGuiCol_Text, text_disabled_imvec4);
        RenderTextClipped(bb.Min + ImVec2(0, -2) + style.FramePadding, bb.Max - ImVec2(5, 0), preview_value, NULL, NULL);
        PopStyleColor( );
    }

    if ( CalcTextSize( preview_value ).x > bb.GetWidth( ) )
        draw->AddRectFilledMultiColor( ImVec2( bb.Max.x - 50, bb.Min.y ), bb.Max, frame_inactive, frame_inactive, frame_inactive, frame_inactive );

    //draw->AddRect( bb.Min, bb.Max, border.to_im_color( ), 3 );

    RenderArrow( draw, ImVec2( bb.Max.x - 13, bb.Min.y + 3 ), text, ImGuiDir_Down, 10 );

    draw->AddText( ImVec2( total_bb.Min.x, bb.GetCenter( ).y - label_size.y / 2 ), text_disabled, label );
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    auto frame_inactive = ImGui::GetColorU32(rgb_to_imvec4(12, 12, 12, 0.9));
    ...
    PushStyleColor( ImGuiCol_PopupBg, frame_inactive );
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("", col_v4, flags, ImVec2(18, 18)))
        {
...
const char* generateRandomString(size_t length) {
    ...
    for (size_t i = 0; i < length; ++i) {
        randomString += charset[std::rand() % (sizeof(charset) - 1)]; // Exclude null terminator
    }

    // Use strdup to create a const char* from the std::string
    return strdup(randomString.c_str());
}
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiID id = window->GetID(generateRandomString(15));
    const float default_size = GetFrameHeight();
    ...
    {
        // Render a square filled with the color
        ImVec4 col_source = (flags & ImGuiColorEditFlags_AlphaPreview) ? col_rgb : col_rgb_without_alpha;
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), rounding);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 1.f); // Set rounding to 0 for a square
    }
    RenderNavHighlight(bb, id);
    //if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    //{
